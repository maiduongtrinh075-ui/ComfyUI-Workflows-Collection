...
> ⚠️ **重要**：JSON 格式**不包含**图片/视频文件本身，只导出消息数据。要下载原图和原视频，**必须使用 HTML 格式**。

#### 访问地址
- **Web UI 入口**：http://localhost:40653/qce-v4-tool
- **Token**：从 `%USERPROFILE%\.qq-chat-exporter\security.json` 的 `accessToken` 字段获取

#### 推荐配置（导出原图+原视频+表情+文件）

| 配置项 | 建议值 | 说明 |
|--------|--------|------|
| **导出格式** | ✅ **HTML** | 唯一会下载原图/原视频的格式 |
| **时间范围** | 自定义 | 按需设置开始/结束时间 |
| 包含系统消息 | ✅ 勾选 | 保留入群通知、撤回提示等 |
| 快速导出（跳过资源下载） | ❌ 不勾选 | 关键！勾选后会跳过所有资源 |
| 仅保留文件元数据 | ❌ 不勾选 | 关键！勾选后只保留文件名不下载 |
| 不下载图片 | ❌ 不勾选 | 关键！勾选后会跳过图片 |
| 不下载视频 | ❌ 不勾选 | 关键！勾选后会跳过视频 |
| 不下载语音 | ✅ 勾选 | 可选，跳过 SILK/AMR 语音 |
| 优先使用群成员名称 | ✅ 勾选 | 群聊用群名片 |
| 文件名包含聊天名称 | ✅ 勾选 | 方便识别 |
| 嵌入头像为Base64 | ❌ 不勾选 | 仅 JSON 格式有效 |
| 流式导出 | ❌ 不勾选 | 仅 50万+ 消息时使用 |

#### 导出目录结构（HTML 格式）

```
%USERPROFILE%\.qq-chat-exporter\exports\
└── QQ_群名_2026-06-13_16-37/
    ├── index.html         # 聊天记录页面（双击直接打开）
    ├── images/            # 📁 所有原图（原始分辨率）
    ├── videos/            # 📁 所有原视频
    ├── files/             # 📁 所有文件
    └── audios/            # 📁 所有语音（如未勾选跳过）
...
**Q: 导出的 JSON 格式里没有图片/视频？**  
A: JSON 格式只导出消息数据，文件路径是引用，不会下载实际文件。要下载原图原视频，必须用 **HTML 格式**。

**Q: HTML 打开后图片/视频无法显示？**  
A: 确保 `images/` 和 `videos/` 文件夹与 `index.html` 在同一目录下，不要单独移动 HTML 文件。

**Q: 导出速度太慢？**  
A: 可以勾选"快速导出"加速，但这样会跳过所有资源下载。或者勾选"不下载视频"跳过视频。

**Q: 文件太大怎么办？**  
A: 可以分时间段多次导出，或者按"文件名包含聊天名称"分类后筛选处理。
